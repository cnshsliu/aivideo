    ...
    switch (body.action) {
      case 'addColumn':
        if (!body.column || !body.type) {
          throw error(
            400,
            'Column name and type are required for addColumn action'
          );
        }
      ...
      case 'modifyColumn':
        if (!body.column || !body.type) {
          throw error(
            400,
            'Column name and type are required for modifyColumn action'
          );
        }
      ...
      case 'addConstraint':
        if (!body.constraintName || !body.constraintType) {
          throw error(
            400,
            'Constraint name and type are required for addConstraint action'
          );
        }
        switch (body.constraintType) {
          case 'primaryKey':
            if (!body.column) {
              throw error(
                400,
                'Column name is required for primary key constraint'
              );
            }
          ...
          case 'check':
            if (!body.checkExpression) {
              throw error(
                400,
                'Check expression is required for check constraint'
              );
            }
          ...
          case 'notNull':
            if (!body.column) {
              throw error(
                400,
                'Column name is required for not null constraint'
              );
            }
      ...
      case 'dropConstraint':
        if (!body.constraintName) {
          throw error(
            400,
            'Constraint name is required for dropConstraint action'
          );
        }
