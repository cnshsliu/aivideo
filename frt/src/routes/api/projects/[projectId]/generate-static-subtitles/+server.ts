...
export async function POST({ params, request, cookies }) {
  try {
    ...
    const result = await executeCommandForSubtitles(command, mainDir);
    ...
    if (result && result.trim()) {
      ...
      const arrayLine = lines.find(line => line.trim().startsWith('[') || line.trim().startsWith('"['));
      if (arrayLine) {
        try {
          ...
          console.error("❌ [STATIC SUBTITLES API] Failed to parse subtitles array:", parseError);
    ...
    console.log("✅ [STATIC SUBTITLES API] Static subtitles generated:", projectId);

    return json({ success: true, result });
  } catch (err) {
    console.error("❌ [STATIC SUBTITLES API] Static subtitles generation error:", err);
    return error(500, { message: "Internal server error" });
  }
}

async function executeCommandForSubtitles(command: string, cwd: string): Promise<string> {
  return new Promise<string>((resolve, reject) => {
    console.log("🚀 [STATIC SUBTITLES API] Executing command:", command);
