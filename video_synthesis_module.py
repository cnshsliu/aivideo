...
class VideoSynthesizer:
    ...
    def add_title(self, clip, args, use_full_duration=False):
        ...
        """
        self.logger.info(f"🔍 DEBUG: VideoSynthesizer.add_title called with title: '{getattr(args, 'title', 'NO TITLE')}'")
        self.logger.info(f"🔍 DEBUG: use_full_duration: {use_full_duration}")
        self.logger.info(f"🔍 DEBUG: clip dimensions: {clip.w}x{clip.h}, duration: {clip.duration}")
        ...
        try:
            ...
            for i, line in enumerate(title_lines):
                ...
                try:
                    ...
                    try:
                        ...
                        title_clip = title_clip.set_duration(title_duration)
                        title_clips.append(title_clip)
                        self.logger.info(f"Successfully created title with fallback font for line: '{line.strip()}'")
                    except Exception as fallback_error:
                        print(f"Damn, fallback title creation also failed: {fallback_error}")
                        self.logger.error(f"Both title font and fallback failed for line: '{line.strip()}'")
                        continue
            ...
            self.logger.info(f"🔍 DEBUG: Creating title_composite with {len(title_clips)} title clips")
            title_composite = CompositeVideoClip(title_clips)
            self.logger.info(f"🔍 DEBUG: Title_composite created: {title_composite.w}x{title_composite.h}")

            if use_full_duration:
                # Create final video with title for full duration
                self.logger.info(f"🔍 DEBUG: Creating full-duration title composite with clip ({clip.w}x{clip.h}) and title_composite")
                final_clip = CompositeVideoClip([clip, title_composite])
                self.logger.info(f"🔍 DEBUG: Full-duration composite created: {final_clip.w}x{final_clip.h}")
            else:
            ...
            self.logger.info(f"🔍 DEBUG: Final clip dimensions: {final_clip.w}x{final_clip.h}, duration: {final_clip.duration}")
            self.logger.info(f"🔍 DEBUG: Title creation completed successfully")
            return final_clip
        ...
        try:
            ...
            if subtitle_clip.w > max_text_width:
                ...
                if self._contains_chinese(subtitle_text):
                    ...
                    emergency_font_size = max(20, int(font_size * 0.6))  # 40% reduction for English
            ...
            self.logger.info(f"Subtitle added successfully at position ({subtitle_x}, {subtitle_y})")
            return final_clip
    ...
    def add_timestamped_subtitles(self, video_clip):
        ...
        if not hasattr(self, 'subtitle_timestamps') or not self.subtitle_timestamps:
            self.logger.warning("No subtitle timestamps available for timestamped subtitles")
            return self._add_subtitles_fallback(video_clip)

        try:
            subtitle_clips = []
            subtitle_position = getattr(self.args, 'subtitle_position', 80)  # Keep using self.args for timestamped subtitles as it's called from main.py
            if subtitle_position is None:
                subtitle_position = 80
            y_position = subtitle_position / 100 * video_clip.h
            font_size = getattr(self.args, 'subtitle_font_size', 48)  # Keep using self.args for timestamped subtitles as it's called from main.py
            if font_size is None:
                font_size = 48
            ...
            for subtitle_info in self.subtitle_timestamps:
                if 'text' in subtitle_info and self._contains_chinese(subtitle_info['text']):
                    needs_chinese = True
                    break
            ...
            for subtitle_info in self.subtitle_timestamps:
                try:
                    ...
                    subtitle_x = video_clip.w / 2 - subtitle_clip.w / 2
                    subtitle_clip = subtitle_clip.set_position((subtitle_x, y_position))
                    ...
                    self.logger.warning(f"Failed to create subtitle clip for '{subtitle_info.get('text', '')}': {e}")
                    continue
    ...
    def apply_transition(self, clip1, clip2):
        ...
        try:
            ...
            transition = CompositeVideoClip([
                clip1_fade,
                clip2_fade.set_start(clip1.duration - transition_duration)
            ])
    ...
    def _check_and_regenerate_aspect_ratio(self, video_file):
        ...
        try:
            ...
            if target_width == 1080 and target_height == 1920:
                ...
                # 16:9 target
                return self._regenerate_with_16_9_aspect_ratio(video_file, width, height)
    ...
    def _regenerate_with_mobile_portrait_ratio(self, video_file, current_width, current_height):
        ...
        try:
            ...
            self.logger.info(f"Cropping video from {current_width}x{current_height} to {new_width}x{new_height}")
            ...
            self.logger.info("Video regenerated with correct mobile portrait aspect ratio")
    ...
    def process_media_clips(self, media_files, args):
        ...
        self.logger.info(f"Processing media clips with args.length: {getattr(args, 'length', 'None')}")
    ...
    def create_final_video(self, video_generator=None):
        ...
        try:
            # Process media clips
            processed_clips = self.process_media_clips()
            ...
            if self.args.title:
                ...
                final_clip = self.add_title(final_clip, use_full_duration=keep_title)

            # Add subtitles
            if hasattr(self, 'subtitle_timestamps') and self.subtitle_timestamps:
                final_clip = self.add_timestamped_subtitles(final_clip)
            elif hasattr(self, 'args', 'subtitle') and self.args.subtitle:
                final_clip = self.add_subtitles(final_clip, self.args.subtitle)

            # Add audio if available
            if hasattr(self, 'audio_file') and self.audio_file and Path(self.audio_file).exists():
                try:
